...
/*   Updated: 2024/06/05 16:08:36 by subson           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

ssize_t static	make_new_str(t_file *file, size_t repeat_num);
ssize_t static	read_next(int fd, char *str, size_t *len, size_t repeat_num);
char static		*parse_by_nl(t_file **file_lst, t_file *file);
...
char static	*parse_by_nl(t_file **file_lst, t_file *file)
{
	...
	nl_index = gnl_strchr(file->str, '\n', file->len);
	if (nl_index == NO_NL)
		result = gnl_substr(file->str, 0, file->len);
	else
	{
		result = gnl_substr(file->str, 0, nl_index + 1);
		backup = gnl_substr(file->str, nl_index + 1, file->len);
